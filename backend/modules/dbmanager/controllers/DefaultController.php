<?php
namespace backend\modules\dbmanager\controllers;
...
class DefaultController extends BaseController
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            ...
            Yii::$app->session->setFlash('crudMessage', Yii::t('dbManager', 'Dump request invalid.') . '<br>' . Html::errorSummary($model));
        }
    ...
    public function actionStorage($id)
    {
        if (Yii::$app->has('backupStorage')) {
            ...
            if ($exists) {
                ...
                Yii::$app->session->setFlash('crudMessage', Yii::t('dbManager', 'Dump deleted from storage.'));
            } else {
                $stream = fopen($dumpPath, 'r+');
                Yii::$app->backupStorage->writeStream($dumpname, $stream);
                Yii::$app->session->setFlash('crudMessage', Yii::t('dbManager', 'Dump uploaded to storage.'));
            }
        }
        return $this->redirect(['index']);
    }
    ...
    public function actionDelete($id)
    {
        $dumpFile = $this->getModule()->path . StringHelper::basename(ArrayHelper::getValue($this->getModule()->getFileList(), $id));
        if (unlink($dumpFile)) {
            Yii::$app->session->setFlash('crudMessage', Yii::t('dbManager', 'Dump deleted successfully.'));
        } else {
            Yii::$app->session->setFlash('crudMessage', Yii::t('dbManager', 'Error deleting dump.'));
        }
        return $this->redirect(['index']);
    }
    /**
    ...
    public function actionDeleteAll()
    {
        if (!empty($this->getModule()->getFileList())) {
            ...
            if (empty($fail)) {
                Yii::$app->session->setFlash('crudMessage', Yii::t('dbManager', 'All dumps successfully removed.'));
            } else {
                Yii::$app->session->setFlash('crudMessage', Yii::t('dbManager', 'Error deleting dumps.'));
            }
        }
        return $this->redirect(['index']);
    }
    ...
    protected function runProcess($command, $isRestore = false)
    {
        ...
        if ($process->isSuccessful()) {
            ...
            Yii::$app->session->addFlash('crudMessage', $msg);
        } else {
            $msg = (!$isRestore) ? Yii::t('dbManager', 'Dump failed.') : Yii::t('dbManager', 'Restore failed.');
            Yii::$app->session->addFlash('crudMessage', $msg . '<br>' . 'Command - ' . $command . '<br>' . $process->getOutput() . $process->getErrorOutput());
    ...
    protected function runProcessAsync($command, $isRestore = false)
    {
        ...
        if (!$process->isRunning()) {
            ...
            Yii::$app->session->addFlash('crudMessage', Yii::t('dbManager', 'Process running with pid={pid}', ['pid' => $pid]) . '<br>' . $command);
        }
    }
    /**
     * @return array
